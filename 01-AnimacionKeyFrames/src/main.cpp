...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	//Cargar la textura del landingpad (pista de atterizaje)
...
void applicationLoop() {
	...
	while (psi) {
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, textureHighwayID);
		esferaPrueba.setPosition(glm::vec3(15.0, 1.0, -5.0));
		...
		modelMatrixLamboLeftDor = glm::translate(modelMatrixLamboLeftDor, glm::vec3(1.08715, 0.704995, 0.976467));
		...
		**************************/
		...
		case 2:
			...
			modelMatrixEclipse = glm::rotate(modelMatrixEclipse, glm::radians(0.1f), glm::vec3(0, 1, 0));
			rotCount += 0.05f;
			rotWheelsX += 0.05;
			rotWheelsY += 0.02;
			if (rotWheelsY > 0.2f)
